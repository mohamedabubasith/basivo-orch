...
async def test_a_warmed_home_is_copied_for_data_and_shared_for_packages(monkeypatch, tmp_path):
    """What the image warms, and why each half is treated differently.

    Sessions are a tenant's, so `data` is copied and a run cannot see another
    run's. The package directory is OpenCode's own 57MB install, the same for
    everyone, so it is shared rather than copied for every repair.
    """
    monkeypatch.setenv("BASIVO_OPENCODE_BIN", str(_fake(tmp_path, "opencode", _OPENCODE_OUTPUT)))
